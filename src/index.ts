...
function filterForCoveredFiles(
  basePath: string,
  files: string[],
  ...
  let newFiles = files;
  if (useAbsolutePath) {
    newFiles = files.map((filename) => path.resolve(basePath, filename));
  }
  return newFiles.filter((filename) => coverage[filename] !== undefined);
}
...
function sendPRComment(config: Config, results: CoverageEntry) {
  ...
  if (!meetsThreshold(results, config.threshold)) {
    ...
    reportFunc(
      config.customFailureMessage !== undefined
        ? config.customFailureMessage
        : defaultMessage
    );
  } else {
    const defaultMessage = `🎉 Test coverage is looking good for ${messageType}`;
    message(
      config.customSuccessMessage !== undefined
        ? config.customSuccessMessage
        : defaultMessage
    );
  }
}
...
function generateReport(
  ...
  const header = `## Coverage in ${getFileGroupShortDescription(
    reportChangeType
  )}
...
---- | ------------: | -----------------: | ----------------: | --------------:`;
  ...
  const lines = Object.keys(coverage.displayed).map((filename) => {
    ...
    const srcPath = useAbsolutePath
      ? path.relative(basePath, filename)
      : filename;
    const shortFilename = formatSourceName(srcPath);
    const linkFilename = formatLinkName(srcPath, branch);
    return [
  ...
  return [header, ...lines, ellided, total, ""]
...
function getCoveragePaths(coveragePaths: SourcePath[]): SourcePathExplicit[] {
  return coveragePaths.map((singleCoveragePath) => {
    ...
    if (typeof singleCoveragePath === "string") {
      ...
      type = singleCoveragePath.match(/(lcov\.info)$/)
        ? "lcov"
        : "json-summary";
    } else {
    ...
    const appDir = `${process.mainModule.paths[0]
...
function getCombinedCoverageCollection(
  coveragePaths: SourcePathExplicit[]
): CoverageCollection {
...
export function istanbulCoverage(config?: Partial<Config>): Promise<void> {
  ...
  const gitProperties = Promise.all([
    gitService.getRootDirectory(),
  ...
  return gitProperties.then((values) => {
    ...
    const useAbsolutePath = combinedConfig.useAbsolutePath;
    const modifiedFiles = filterForCoveredFiles(
      gitRoot,
      danger.git.modified_files,
      coverage,
      useAbsolutePath
    );
    const createdFiles = filterForCoveredFiles(
      gitRoot,
      danger.git.created_files,
      coverage,
      useAbsolutePath
    );
    const allFiles = Object.keys(coverage).filter(
      (filename) => filename !== "total"
    );

    const files = getFileSet(
      combinedConfig.reportFileSet,
      allFiles,
      modifiedFiles,
      createdFiles
    );
